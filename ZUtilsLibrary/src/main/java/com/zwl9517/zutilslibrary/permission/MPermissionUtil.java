package com.zwl9517.zutilslibrary.permission;
...
final public class MPermissionUtil {
    ...
    public static boolean checkCameraPermission(Activity activity, int requestCode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            //23以下的版本不需要检查权限，如果检查，会抛出无该方法异常
            List<String> deniedPermissions = MPermissionUtil.findDeniedPermissions(activity, Manifest.permission.CAMERA);
            if (!deniedPermissions.isEmpty()) {
                boolean isGrant = true;
                for (String deniedPermission : deniedPermissions) {
                    if (deniedPermission.equals(Manifest.permission.CAMERA)) {
                        MPermission.needPermission(activity, requestCode, Manifest.permission.CAMERA);
                        isGrant = false;
                    }
                }
                return isGrant;
